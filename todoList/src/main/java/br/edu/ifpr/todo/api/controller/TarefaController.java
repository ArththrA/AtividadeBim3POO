...
public class TarefaController {
    ...
    @GetMapping
    public List<TarefaResponse> listar(@RequestParam(required = false) TodoStatus status, @RequestParam(required = false) Boolean importante) {
        List<Tarefa> tarefas = service.listar(null, status, importante, null);
        List<TarefaResponse> resposta = tarefas.stream().map(this::toResponse).toList();
        return resposta;
    }
